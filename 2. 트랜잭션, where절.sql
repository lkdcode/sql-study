...
-- 성씨가 김씨이면서, 부서가 1000003, 1000004, 1000006 번 중에
-- 하나이면서
...
FROM TB_EMP
WHERE SEX_CD <> 2
--WHERE NOT SEX_CD = 2 부정 연산자
--WHERE SEX_CD != 2 부정 연산자
--WHERE SEX_CD ^= 2 부정 연산자
;
...
FROM TB_EMP
WHERE DIRECT_MANAGER_EMP_NO IS NOT NULL -- NULL 이 아닌 경우
WHERE DIRECT_MANAGER_EMP_NO NOT NULL -- NULL 인 경우
;
